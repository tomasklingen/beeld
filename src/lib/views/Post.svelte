<script lang="ts">
    ...
    const onError = (post: RedditPost) => (e) => {
        console.log("failed to load post", post)
    }
...
<div class="post">
    {#if hasEmbed(post) }
        {#if showEmbed}
            <iframe allowfullscreen class="embed" src={post.secure_media_embed.media_domain_url} title='embed'/>
        {:else}
            <!-- svelte-ignore a11y-click-events-have-key-events -->
            <img src={post.thumbnail} alt='img' on:click={() => showEmbed = true} />
        {/if}
    {:else if isGifv(post)}
        <video preload="metadata" poster={post.thumbnail} controls muted loop on:error={onError(post)}>
